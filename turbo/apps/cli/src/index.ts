...
authCommand
  ...
  .action(async () => {
    try {
      ...
      console.log(chalk.yellow(`And enter this code: ${chalk.bold(deviceAuth.user_code)}`));
  ...
  .action(() => {
    try {
      ...
      if (process.env.USPARK_TOKEN) {
        console.log(chalk.blue("ℹ Using token from USPARK_TOKEN environment variable"));
      }
    } catch (error) {
      console.error(chalk.red(`✗ Failed to check status: ${error instanceof Error ? error.message : error}`));
      process.exit(1);
    }
  });
