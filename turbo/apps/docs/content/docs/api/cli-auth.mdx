...
```bash
curl -X POST https://uspark.ai/api/cli/auth/device \
...
```bash
# Request device code
RESPONSE=$(curl -X POST https://uspark.ai/api/cli/auth/device)
DEVICE_CODE=$(echo $RESPONSE | jq -r '.device_code')

# Poll for token
while true; do
  TOKEN_RESPONSE=$(curl -X POST https://uspark.ai/api/cli/auth/token \
    -H "Content-Type: application/json" \
...
```bash
curl -X POST https://uspark.ai/api/cli/auth/generate-token \
...
echo "Visit https://uspark.ai/cli-auth and enter code: WDJB-MJHT"
...
type DeviceAuthResponse = {
  ...
  verification_url: string; // https://uspark.ai/cli-auth
...
RESPONSE=$(curl -s -X POST https://uspark.ai/api/cli/auth/device \
...
while true; do
  TOKEN_RESPONSE=$(curl -s -X POST https://uspark.ai/api/cli/auth/token \
    -H "Content-Type: application/json" \
    -d "{\"device_code\": \"$DEVICE_CODE\"}")
