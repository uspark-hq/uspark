...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const project of projects) {
        ...
        try {
          ...
          if (!blobInfo) {
            console.log(
              `Skipping project ${project.id}: cron.md blob not found`,
            );
            result.skippedProjects.push({
              projectId: project.id,
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to download cron.md: ${response.statusText}`,
            );
          }
    ...
    console.log(
      `Cron job completed: processed ${totalProjects} projects total`,
    );
