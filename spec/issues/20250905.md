...
**任务描述**:
- 实现 `uspark push <file-path> --project-id <id>` 基础功能
- 仅处理单个文件上传（不含 --all 参数）
- 复用现有 FileSystem 类的 pushFile 方法
- 添加进度提示和错误处理
...
### 4. 实现 Blob STS Token API 🔑

**负责模块**: `turbo/apps/web/app/api/projects/[projectId]`

**任务描述**:
- 创建 `GET /api/projects/:projectId/blob-token` 端点
- 生成 10 分钟有效期的临时访问令牌
- 返回 Vercel Blob 的上传/下载 URL
- 验证用户对项目的访问权限

**验收标准**:
- [ ] API 端点正确响应并返回 token
- [ ] Token 包含有效的上传/下载 URL
- [ ] 非授权用户收到 403 错误
- [ ] 包含 API 测试用例

**相关文档**: 
- `spec/issues/mvp.md` (Phase 1 - Blob STS token)
- `spec/issues/yjs.md` (Blob STS Token API)
...
1. **API 修复** (任务 1, 4): 后端 API 改进
2. **CLI 功能** (任务 2, 3, 6): CLI 相关功能和优化
...
- 任务 3: 实现 push 命令（核心 CLI 功能）
- 任务 4: Blob STS Token（解锁文件访问）
