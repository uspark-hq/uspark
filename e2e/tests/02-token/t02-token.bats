...
@test "CLI auth login initiates device flow" {
    ...
    timeout 2 bash -c 'echo | API_HOST="$API_HOST" uspark auth login' > /tmp/auth_init_output 2>&1 || true
...
@test "CLI auth login shows device flow instructions" {
    ...
    timeout 2 bash -c 'echo | API_HOST="$API_HOST" uspark auth login' > /tmp/auth_output 2>&1 || true
